...
- **项目列表** `/projects`：展示当前用户成员项目、`owner/admin/member` 角色和审核能力，支持创建项目及输入邀请码加入项目。创建表单复用类别方案编辑器，默认预填 RFID-CV 规范 12 类并允许编辑，但至少一个类别的名称、分组、颜色、参与对象模式及对应数量/角色完整前不能提交；创建成功后进入未锁定的项目管理页复核，永久锁定仍需 owner 另行确认。
...
- **标注工作台** `/projects/:projectId/annotate/:videoId`：
  ...
  - 行为标注列表支持 PATCH 类别/时间、DELETE 删除；行为行精简为仅行为身份（类别、时间、参与对象），不再渲染帧号 / 标注者 / 可信度 / 审核状态元数据，也不用彩色底色强调已通过 / 需修改状态；「编辑 / 删除」为显眼且可访问的原生按钮，仅保留角色待补全 / Track 失效等待处理提示。
  - **直接编辑现有行为标注**（含退回后修改）：进入编辑时类别、起止帧与参与对象同步到共享编辑状态，类别数字键、S/D、参与对象键盘选择（T / ↑ / ↓ / Enter）、Space 与 ←/→ 传输控制继续生效；Ctrl+Enter 与「保存编辑 [Ctrl+Enter]」按钮 PATCH 保存当前标注，而不是新建草稿。输入框聚焦保护与 Esc 取消编辑不变，Delete / Ctrl+Z / ? 及裸 Tab 模式切换在编辑态不触发；已通过并锁定的行为标注仍不可编辑，切换视频仍走确认。
  - 标注页不再提供单视频“导出 JSON”下载按钮；legacy `GET .../annotations/export`、前端 API wrapper 与类型继续保留以兼容既有调用，但不替代正式项目 ZIP 导出页。
  - 保存中 / 已保存 / 失败状态提示。
  - **批次 3 审核工作流**：顶部清晰显示视频工作流状态与行为标注版本（草稿 / 待审核 / 已通过 / 已退回，即 `draft/submitted/approved/rejected`，以及提交/通过时间）；`draft/rejected` 可「提交审核」（至少一条标注、有 detection import、无 `needs_mouse_ids`，有确认），`submitted` 显示「等待审核」，`approved` 显示「已通过」。最近一次审核结果为退回，且视频为 `rejected` 或开始修改后变回 `draft` 时，标注工作台主区播放器之前的退回面板持续展示该次退回意见、审核人和时间；正文超出固定可读高度时仅在面板内纵向滚动，重新提交为 `submitted`、通过或后续审核已覆盖该退回结果时隐藏。单条行为标注的 `review_status` 是独立的 `pending/approved/rejected`，不得与视频工作流混用。标注 CRUD 的非草稿失效与 track 修正的细粒度失效范围不同：track 修正后全部 Annotation 被重校验，有效项推进修订，无效项 `needs_mouse_ids`；仅实际受影响的 approved 单条标注改 pending，视频仅在 submitted/approved 时退回 draft，不声称全部审核状态重置。
  - **退回意见 / 退回行为面板**：面板回到 `annotate-main` 常规文档流、固定位于播放器之前，具有固定可见高度且仅面板体内部纵向滚动，不覆盖视频、时间轴与主控件，窄屏随主区堆叠；它不再是右侧侧栏，`annotate-side` 不承载该面板。存在逐条 `feedback_items` 时以「退回行为」列出每条退回时的版本、当前快照、`comparison` 对照标签、审核人和时间；无逐条记录时回退展示最近一次视频退回意见。面板只有两个用户操作：点击条目（原生 button）用该条退回行为替换当前唯一选中，选中对应实时行为并把播放头跳到其起点、时间轴同步高亮，行为列表与时间轴因此至多一个选中行为；行为已删除时不改变当前选中，仅提示无法定位、不选中幽灵 ID；以及「标记已修改」。标记是独立于 `comparison=modified` 的人工标记（界面文案分别为「标记已修改」与对照标签「已修改」），任意可访问该视频的 active 项目成员均可标记；标记幂等并保留首次标记时间，不改变提交门禁。标记后条目乐观地按「未标记在前、已标记在后（组内按标记时间）」下沉到列表底部，失败回滚本地乐观标记。定位不自动进入编辑。
...
- **审核工作台** `/projects/:projectId/review`：
  - 桌面端为三列审核台：窄左栏放视频导航与视频级裁决，中间固定播放器和时间轴，右栏在视频旁独立纵向滚动完整行为列表与单条意见；任何意见输入都不会覆盖播放器。
  - 左栏顶部在「待审核队列 / 查找历史/复核视频」间切换；历史/复核列表使用 `view=all` 保留 approved、rejected 以及当前投影为 draft 的 withdrawn 提交，并支持文件名查找。顶部不再重复提供视频下拉框。窄桌面保持播放器与右侧行为列表并排并把左栏移到上方，移动端视频导航为抽屉、行为列表堆叠展示。
  - 意见输入 + 「通过 / 退回」（退回须填意见，均有确认）；审核历史默认折叠。审核页不展示或轮询视频片段生成状态，但审核通过后后台生成能力保持不变。
...
- **片段库** `/projects/:projectId/clips`（**批次 5**，全部项目成员可见）：
  ...
  - **顶部共享预览区**：点击片段后通过共享完整 GET/Blob 能力加载，跳转到片段 `start_time`，播放范围限制在 `[start_time, end_time]`（到点自动暂停并提示）；一次只播放一个，绝不批量预加载视频。切换片段会终止旧媒体世代并撤销旧 object URL。范围条高亮片段区间，点击 / 键盘（←/→）自由跳转。
  - **「跳转到标注」**回 `/projects/:pid/annotate/:vid?t=start`，标注工作台读取 `?t=` 自动定位播放头。
  - 片段卡片：仅在 `clip_id` 存在且 `media_status=ready` 时经 `/api/projects/{pid}/clips/{clipId}/thumbnail` 以 Bearer 拉取缩略图；失败时回退 SVG 占位。卡片身份使用服务端不透明 `item_key`，生成状态完全由 `media_status` 驱动，不读取磁盘路径。
...
视频展示源不由 Vite 环境变量选择；统一由后端 `DISPLAY_PROXIES_ENABLED` 在运行时决定。
