...
- **审核工作台** `/projects/:projectId/review`：
  - 桌面端为固定双列审核台：左侧 sticky 裁决栏默认显示当前行为摘要、意见、通过/退回和折叠审核历史，右侧保持播放器、时间轴、类别筛选和局部滚动的完整行为列表。
  - 左栏顶部为「审核队列 / 审核裁决」切换标签：点击“审核队列”在同一左栏显示队列，点击“审核裁决”回到裁决内容，均不增加列或挤压视频；首次未选视频时默认打开队列，选择视频后自动收起，`Esc` 关闭后回焦队列标签。窄屏改为带遮罩的抽屉。
  - 当前行为摘要复用筛选、active 并集和单条聚焦语义，展示类别、起止、参与对象及角色，不复制完整行为列表。
