...
> **历史设计，非当前权威。** 本文保留旧物化 track、集中式导出及迁移前业务规则，供回滚和数据迁移追溯；不得用于推导当前运行时模型或正式导出契约。`CorrectedTrack`、`CorrectedDetectionAssignment`（CDA）、`IdentityEdit`、`DetectionSuppression` 与 `SuppressionDetection` 仅是迁移兼容背景。当前权威见[现行架构设计](../设计/检测状态、提交审核与独立行为视频片段导出设计.md)。
>
> 历史状态：旧方案曾实现并验收；其后已被 Submission authority 与独立四文件导出替代。
> 日期：2026-08-07
...
8. **后续结果**：该方案被现行架构替代后，真实 ffmpeg/ffprobe 25/30/60 FPS 验收已通过；公网入口仍受阿里云备案接入阻塞。

最终自动化证据已更新为：后端全量 `397 passed, 3 skipped`；前端 `npm run build` 通过。本文其余验收叙述仍属于被替换方案的历史记录。
...
以下是该旧方案被替代前记录的剩余事项，仅保留历史脉络，不是当前执行清单：

1. 当时计划使用正式原始视频 `社交-攻击1.mov` 完成长视频和浏览器完整流程；真实 ffmpeg 验收后来已由现行架构的 25/30/60 FPS 证据覆盖；
