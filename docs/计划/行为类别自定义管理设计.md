...
- 创建表单的本地草稿可从空列表开始，但提交 `POST /api/projects` 时必须携带至少一个完整类别；Project、owner membership、规范化类别和初始 replace audit 原子创建，初始 version=0 且保持未锁定。
...
- 创建表单的本地草稿可从空列表开始，但类别未完整前禁止提交；普通项目必须与至少一个包含名称、分组、非空颜色、参与对象模式及对应数量/角色的完整类别方案原子创建，不再预置默认 12 类。
...
- 已持久化的普通项目不得以零类别方案作为当前契约；“空列表”仅是创建表单尚未提交的本地草稿状态。
...
- 受控 `seed_demo.py` 的 Project、owner、12 类、replace/lock audit 与永久锁定同事务提交，不构成普通项目默认值。
...
- 创建表单草稿可为空，但项目必须与非空完整类别方案原子创建；初始 version=0、未锁定，非法请求不落库，未锁定禁止所有 Annotation 写。
