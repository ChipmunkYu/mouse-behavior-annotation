...
> 状态：现行架构设计，已实现并完成后端与真实媒体验收

> 最终证据：后端 `397 passed, 3 skipped`；真实 FFmpeg/ffprobe 25/30/60 FPS 验收通过。多地区 HTTPS 已成功，HTTP 及部分来源仍受备案同步影响，完整公网验收待完成。
> 日期：2026-08-11
> 对应工作项：WI-20260805-22

## 1. 状态、范围与目标

当前 draft 检测读取与 Split/Merge/Suppress/LIFO Undo 使用 `RawDetection + DetectionStateOverride + DraftIdentityEdit/DraftDetectionChange`。submit/withdraw/review、Review、Clip/media job 和片段库新数据使用不可变 `Submission + DetectionSnapshot + SubmissionAnnotation` authority；legacy Review/Clip 仅只读兼容，旧表保留迁移与回滚背景。独立四文件 ZIP 打包已经实现并通过真实媒体验收。
...
本节契约已完成后端实现；25/30/60 FPS 已通过真实 ffmpeg/ffprobe 验收。入队时 approved、之后 superseded 的 Submission
...
最终后端全量结果为 `397 passed, 3 skipped`。真实 ffmpeg/ffprobe 验收覆盖：

- 25 FPS：51 帧，320×240，2.04 秒；
- 30 FPS：61 帧，240×180，约 2.034 秒；
- 60 FPS：61 帧，160×120，约 1.017 秒。

每例均验证 crop 后尺寸、缩略图可解码、片段首末帧可解码以及帧数/时长约束。该证据覆盖真实媒体契约，但不代表完整公网入口已验收；多地区 HTTPS 已成功，HTTP 及部分来源仍受备案同步影响，完整公网验收待完成。
