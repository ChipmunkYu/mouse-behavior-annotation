...
> 状态：现行架构设计；媒体修复已实现并通过自动测试、真实 FFmpeg 编码与 ffprobe 属性本地验证，生产版本与部署验收待完成

> 当前证据：此前后端全量 `399 passed, 8 skipped`。2026-08-17 在 Python 3.11.9 隔离环境中，以 imageio-ffmpeg 0.6.0 内置 FFmpeg `7.1-essentials_build-www.gyan.dev`（含 libx264）和 npm `@ffprobe-installer/win32-x64@5.1.0` 的 ffprobe 5.1 兼容包运行真实集成测试，结果 `5 passed, 1 warning`；媒体与项目导出聚焦回归为 `66 passed, 1 warning`，均无 skip。已验证 25/30/60 FPS 媒体属性、发布边界与 MediaWorker 状态；warning 为既有 Starlette/httpx deprecation warning。生产 FFmpeg/ffprobe 4.4.2 兼容性仍未验证，`fix/clip-transcoding@bcc9a03` 未合并、未部署。
> 日期：2026-08-11
> 对应工作项：WI-20260805-22

> 2026-09-09 行为级审核增量：本分支已实现 `0017`，逐条裁决、跨轮次锁定/沿用、改后恢复阻断、显式撤销/reopen、legacy 和 purge 的现行细则见[行为级审核 API 契约](../behavior-review-api.md)。本文原视频级裁决流程须叠加这些门禁；生产状态不由此更新。

## 1. 状态、范围与目标

当前 draft 检测读取与 Split/Merge/Suppress/LIFO Undo 使用 `RawDetection + DetectionStateOverride + DraftIdentityEdit/DraftDetectionChange`。submit/withdraw/review、Review、Clip/media job 和片段库新数据使用不可变 `Submission + DetectionSnapshot + SubmissionAnnotation` authority；legacy Review/Clip 仅只读兼容，旧表保留迁移与回滚背景。独立四文件 ZIP 打包已经实现并通过自动测试；真实媒体编码、ffprobe 属性与 worker 主路径已在本地完整验证，仍待生产 FFmpeg/ffprobe 4.4.2 候选环境验收。

本文把检测状态编辑、提交审核、媒体片段和独立行为视频片段导出统一为一套确定、可实施的设计。目标是：draft 编辑轻量且可并发保护；提交和审核读取不可变快照；导出与审核严格同源；每个已批准行为标注可导出为不依赖集中索引的自包含目录。

source SHA-256 在 Video gate 外计算，gate 内仅复核 DB 修订/key 与文件 size+mtime_ns；snapshot 以 raw/state/metadata digest 防同 count 篡改。行为帧端点双 inclusive，媒体计划转换为半开区间 `[start/fps, (end+1)/fps)`，clip/thumbnail 共享 crop；track 坐标转换复用同一时间/crop authority。
...
3. draft 可 Split、Merge、抑制和 LIFO undo；`submitted` 锁编辑，要继续编辑必须先 withdraw。`rejected/withdrawn` 中已通过行为仍锁定普通修改，须审核人显式撤销；最终通过必须先 reopen 才能撤销/重提。track-only 不撤销通过、不解决退回意见，快照始终不可变。
...
6. effective raw/display track ID 统一满足 `0 <= id < 9223372036854775807`；`next_display_track_id` 游标满足 `0 <= cursor <= 9223372036854775807`。游标等于上界表示 ID 空间已耗尽，分配必须拒绝且不得再加 1。
...
flowchart LR
    ...
    A -->|explicit reopen| X[Old approved superseded]
    X -->|draft edits + new submit/approve| N[New approved Submission]
```
...
以下是现行模型的架构约束。所有时间使用项目统一 UTC 时间约定；布尔值和枚举在 ORM 与数据库层同时校验。SQLite 外键必须启用。
...
检测编辑条件是视频没有 `submitted` Submission；track-only 保留已有行为裁决。普通行为 PATCH/DELETE 另受最新审核尝试的已通过锁定约束；最终通过后新增/修改/删除行为、检测基线替换、撤销和再次提交均须先显式 reopen。视频投影误降为 draft/rejected 也不能绕过 approved Submission 的写入或 purge 门禁。所有编辑写事务使用 SQLite 短事务写门禁、`expected_edit_version` CAS、唯一约束和必要索引。

当前实现同时检查权威 `Submission(status=submitted)` 与兼容投影 `Video.workflow_status=submitted`；命中时明确返回 409，必须先 withdraw 才能继续编辑。成功编辑后，`Video.identity_revision` 与语义未变化 Annotation 的 `identity_revision` 仅作为 `DetectionImport.edit_version` 的兼容投影。
...
stateDiagram-v2
    ...
    approved --> superseded: explicit reopen
    withdrawn --> [*]
    rejected --> [*]
    superseded --> [*]
```
...
- approved 经显式 reopen 原子改为 superseded，追加重新打开及逐条撤销审计；后续编辑与新提交不修改旧 snapshot。
...
本节契约已完成后端实现并通过自动测试及本地真实 FFmpeg/ffprobe 验证；25/30/60 FPS 与产物属性本地验收均已通过，生产 FFmpeg/ffprobe 4.4.2 候选验收尚未完成。入队时 approved、之后 superseded 的 Submission
...
- Clip 使用 `-ss` 定位与 `-t` 指定片长，全程重编码为 `libx264 + yuv420p + faststart`；默认 `-an` 不输出音频，启用 `MEDIA_MAP_AUDIO` 时可选映射首条音轨并编码为 AAC。
- 不提供 stream-copy 降级。无法按上述契约编码时任务失败，不能以复制源流换取表面成功。
- worker 先写 `.mp4.part` 和 `.jpg.part` 再原子发布。FFmpeg 无法从最后扩展名 `.part` 推断 muxer 是本次“导出无法重编码”的根因；命令现已分别显式指定 `-f mp4` 与 `-f image2`。

### 6.10 失败恢复与导出调度契约

- 启动恢复发现 interrupted media job 时，无论任务可重排还是已耗尽重试并转为 `failed`，都必须将该任务遗留的 `processing` Clip 以 CAS 释放为 `pending`，使显式重试或 export 补生成能够重新 claim。
- export job 在事务提交后交给 worker。若 schedule handoff 抛错，仅当该 job 仍为 `queued` 且仍持有项目 active key 时，才将其置为 `failed`、释放 active key，并向请求返回 503。
- 若 worker 已将 job claim 为 `running`，上述 guarded failure update 不得改写状态、错误、完成时间或 active key；后续并发导出仍由 active key 返回 409。

### 6.11 本次明确未纳入

- 未实现源视频 `needs_transcode` 的自动转码，也未实现片段库播放能力。
- crop 语义扩展与 cleanup 范围扩展均未纳入本次媒体修复。

## 7. 迁移兼容与回滚边界

旧 `CorrectedTrack`、`CorrectedDetectionAssignment`（CDA）、`IdentityEdit`、`DetectionSuppression` 与 `SuppressionDetection` 不再是当前运行时 authority，仅为迁移兼容与回滚背景保留。原始模型、切换步骤和历史限制见[历史 YOLO 设计](../历史/YOLO检测结果接入与track修正设计.md)与[历史业务逻辑伪代码](../历史/ID修正与行为标注业务逻辑伪代码.md)。

迁移遵循加法变更、严格预检、old/new shadow 对比、停止旧 writer 后切换、保留至少一个发布周期回滚能力、确认备份后再清理旧表的顺序。不得长期双写，也不得以当前 Annotation 伪造旧审核快照。
...
- SQLite batch DDL 失败不能依赖事务 rollback；L3 必须停服并制作一致性备份，迁移或专项验证失败时保持停服、恢复该备份后再重试。
- 不长期双写；切换失败时停写、恢复备份或回到仍保留的旧读写路径。
- 所有 detection edit、Annotation create/update/delete、submit 与 detection replacement 竞争同一 Video 写门禁；锁内重读 authority 与状态，SQLite busy/locked 对外统一为可重试 409。

### 7.3 YAGNI

当前架构不采用 event sourcing、任意 undo/redo、每 edit 全量 snapshot、detection IDs 巨型 JSON、`TrackPoint`、通用资产平台或集中导出索引。内部操作数据不进入独立导出。

## 8. 已验证证据

此前自动测试证据为后端全量 `399 passed, 8 skipped`。命令构造测试覆盖显式 muxer、`-ss + -t`、默认无音频/可选 AAC、失败恢复与导出调度 guard；Fake processor 测试证明状态机和四文件打包。

2026-08-17 在 Python 3.11.9 隔离环境中，`.venv\Scripts` 已可找到 imageio-ffmpeg 0.6.0 内置的 FFmpeg `7.1-essentials_build-www.gyan.dev`（含 libx264）和 npm `@ffprobe-installer/win32-x64@5.1.0` 提供的 ffprobe 5.1 兼容包。真实集成测试结果为 `5 passed, 1 warning in 2.41s`，媒体与项目导出聚焦回归为 `66 passed, 1 warning in 51.90s`，均无 skip；warning 为既有 Starlette/httpx deprecation warning。真实验证覆盖 25/30/60 FPS、H.264、yuv420p、300x200 crop、各 10 帧、约 `10/fps` 时长和 JPEG 300x200，并证明成功后无 `.part`/`.staging`、缩略图注入失败不发布最终文件，以及完整 MediaWorker 将 Job/Clip 更新为 `succeeded`/`ready`。生产服务器 FFmpeg/ffprobe 4.4.2 兼容性仍未验证，不构成生产候选验收；`fix/clip-transcoding@bcc9a03` 未合并 `main`、未部署，既有服务器部署与公网状态不变。

同日真实小鼠三文件 E2E 进一步验证了设计主链：30 FPS/156 帧原视频、tracks JSONL 与 metadata 仅通过公开 API 导入 1877 条检测，真实 track 的 15 帧标注依次完成 Submission 冻结、approved Review、异步 Clip ready 和项目 Export succeeded。下载 ZIP 的单片段目录严格只有 `clip.mp4`、`tracks.json`、`annotation.json`、`metadata.json`，各 JSON 与检测/标注计数一致；MP4 为 H.264、yuv420p、2044×1080、15 帧、0.5 秒，发布完成后无 `.part`/`.staging`。该证据来自被忽略的本地 `backend/data/local-e2e`（SQLite `0011`），不是提交内容，也不改变生产待验收边界。
