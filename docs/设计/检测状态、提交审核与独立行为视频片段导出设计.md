...
> 最终证据：后端 `397 passed, 3 skipped`；真实 FFmpeg/ffprobe 25/30/60 FPS 验收通过。公网仍受阿里云备案接入阻塞，不属于已验收范围。
> 日期：2026-08-11
...
每例均验证 crop 后尺寸、缩略图可解码、片段首末帧可解码以及帧数/时长约束。该证据覆盖真实媒体契约，但不代表公网入口已验收；公网仍受阿里云备案接入阻塞。
