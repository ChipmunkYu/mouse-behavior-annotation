...
> 状态：现行架构设计；媒体修复已实现并通过自动测试与部分真实 FFmpeg 验证，属性、生产版本与部署验收待完成

> 当前证据：此前后端全量 `399 passed, 8 skipped`、聚焦 `63 passed, 5 skipped`；两项统计早于本次仅运行的真实集成测试。本次 imageio-ffmpeg `7.1-essentials_build-www.gyan.dev` 测试为 `2 passed, 3 skipped`，已验证真实编码/失败发布边界与 MediaWorker 状态更新；因无 ffprobe，媒体属性仍未验证。该二进制不是生产服务器 FFmpeg 4.4.2，本修复尚未部署。
> 日期：2026-08-11
> 对应工作项：WI-20260805-22

## 1. 状态、范围与目标

当前 draft 检测读取与 Split/Merge/Suppress/LIFO Undo 使用 `RawDetection + DetectionStateOverride + DraftIdentityEdit/DraftDetectionChange`。submit/withdraw/review、Review、Clip/media job 和片段库新数据使用不可变 `Submission + DetectionSnapshot + SubmissionAnnotation` authority；legacy Review/Clip 仅只读兼容，旧表保留迁移与回滚背景。独立四文件 ZIP 打包已经实现并通过自动测试；真实媒体编码与 worker 主路径已部分验证，完整属性仍待在目标 FFmpeg/ffprobe 环境验收。
...
本节契约已完成后端实现并通过自动测试与部分真实 FFmpeg 验证；因缺少 ffprobe，25/30/60 FPS 与产物属性尚未验收。入队时 approved、之后 superseded 的 Submission
...
此前自动测试证据为：后端全量 `399 passed, 8 skipped`；媒体与项目导出聚焦测试 `63 passed, 5 skipped`。这两项统计早于本次只运行的 `test_media_ffmpeg_integration.py`。命令构造测试覆盖显式 muxer、`-ss + -t`、默认无音频/可选 AAC、失败恢复与导出调度 guard；Fake processor 测试证明状态机和四文件打包。

本次使用 imageio-ffmpeg `7.1-essentials_build-www.gyan.dev` 的真实集成测试结果为 `2 passed, 3 skipped`：已真实证明 `.mp4.part` 配合 `-f mp4` 编码、`.jpg.part` 配合 `-f image2` 生成、缩略图失败时不发布最终文件，以及完整 MediaWorker 将 Job/Clip 更新为 `succeeded`/`ready`。由于没有 ffprobe，25/30/60 FPS 三例及 codec、pixel format、尺寸、帧数、时长属性仍未验证。该二进制不是生产服务器 FFmpeg 4.4.2，不构成生产兼容验证；本修复尚未部署，既有服务器部署与公网状态不变。
