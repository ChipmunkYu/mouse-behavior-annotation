...
P7_TABLES = frozenset({"feedback_marks"})
...
def inspect_state(database_url: str) -> str:
    ...
    # ---- 无有效版本行（无版本表或空版本表）----
    unexpected = tables - P1_TABLES - P2_TABLES - P3_TABLES - P4_TABLES - P5_TABLES - P6_TABLES - P7_TABLES - {VERSION_TABLE}
    if unexpected:
        raise MigrationStateError(
            f"数据库存在非预期表 {sorted(unexpected)}，无法安全判定迁移状态，"
            "拒绝 stamp / 升级（请人工检查后再处理）"
        )
    has_post_baseline = bool((P2_TABLES | P3_TABLES | P4_TABLES | P5_TABLES | P6_TABLES | P7_TABLES) & tables)
    if has_post_baseline:
        # 有增量表却无有效版本行：状态不一致（如 head 库版本行被清空），不可盲 stamp
        raise MigrationStateError(
            f"数据库含增量表 {sorted((P2_TABLES | P3_TABLES | P4_TABLES | P5_TABLES | P6_TABLES | P7_TABLES) & tables)} 却无有效版本行，"
            "迁移状态不一致，拒绝自动 stamp"
        )
    if P1_TABLES & tables:
