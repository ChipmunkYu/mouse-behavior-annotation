...
- 分工能力：`owner/admin/member` 三角色、member 独立 `can_review`、项目邀请码、视频当前负责人、原子领取/释放、事务批量分配、三视图与统计；分工不隔离 active 成员的编辑权限。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| `GET` | `/api/projects/{project_id}/videos` | 项目视频列表；支持 `view=mine/unassigned/all`、工作流和负责人筛选 |
...
| `POST` | `/api/projects/{project_id}/videos/{video_id}/claim` | 项目成员原子领取未分配视频 |
...
| `GET` | `/api/projects/{project_id}/assignment-stats` | 项目与逐负责人的工作流数量统计 |
...
当前最终全量结果：`414 passed, 3 skipped`；分工模块最后一次 focused 结果：`17 passed`。前端 `npm run build` 通过。真实 ffmpeg/ffprobe 的 25/30/60 FPS 验收也已通过；这些证据不替代部署、公网入口、备份恢复或持续的浏览器回归验收。
...
分工模块（三角色与 `can_review` 权限、成员管理、邀请码幂等加入/重置、精简负责人目录、并发领取、draft 释放、事务批量分配、三视图、负责人筛选、统计、上传与三文件导入指定负责人、复合外键与 active trigger），
