...
- P4 低码率展示代理已在当前分支本地完成：新 file-backed Video 在唯一开关开启时计算 hash 并入队；四入口统一认证完整 GET、`response.blob()` 与 object URL；pending/failed 严格返回 409；片段、Submission 和导出始终读取原片。代码尚未提交、推送或部署，不能表述为已上线。
