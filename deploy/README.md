...
5. 从该 release 的 `backend/` 工作目录显式运行 `.venv/bin/python scripts/migrate.py`，运行前让当前进程导入上述稳定环境文件。迁移脚本会实例化同一 `Settings`，因此无效生产凭据会在接触数据库前失败。Pydantic 从当前工作目录读取的 `.env` 只方便开发，不能作为生产 secret 方案。
6. 迁移和构建成功后，才将 `/opt/mouse-annotation/current` 原子切换到该 release。
7. 安装 `deploy/mouse-annotation.service` 后由 systemd 启动后端。`WorkingDirectory` 保持为 `current/backend`，systemd 从稳定路径 `/home/jinghan/.config/mouse-annotation/backend.env` 注入环境；Uvicorn 仅监听 `127.0.0.1:8000` 且保持单进程。
