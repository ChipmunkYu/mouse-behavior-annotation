...
当前仅完成本地代码，尚未提交、推送或在服务器执行。生产实时状态仍以“网站服务器文件清单”为准。P4 必须使用维护窗口，不得滚动发布或运行双实例：
...
服务器 FFmpeg/ffprobe 4.4.2 真实代理、秋采长视频/时间轴、真实 ENOSPC、并发资源以及 Chrome/Edge/Firefox 大 Blob 仍是外部门禁；它们不阻止推送代码候选，但未通过前不得宣称已上线。
